...
def convlstm_1d_basic(hp):
    model = keras.Sequential()
    model.add(ConvLSTM1D(filters=hp.Int('convlstm1_filters', min_value=32, max_value=128, step=32), 
    ...
    model.add(ConvLSTM1D(filters=hp.Int('convlstm2_filters', min_value=64, max_value=256, step=32), 
                         kernel_size=hp.Choice('convlstm2_kernel', values=[3, 5]), 
                         data_format='channels_last',
                         padding="same",
